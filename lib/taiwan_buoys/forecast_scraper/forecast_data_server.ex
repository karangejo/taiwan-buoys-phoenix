defmodule TaiwanBuoys.ForecastScraper.ForecastDataServer do
  @moduledoc """
  this module is responsible for keeping
  the forecast data
  """
  use GenServer

  alias TaiwanBuoys.DataSources
  alias TaiwanBuoys.ForecastScraper.Forecast

  # Client

  def start_link(_) do
    GenServer.start_link(__MODULE__, %{}, name: __MODULE__)
  end

  def put_data(data) do
    GenServer.call(__MODULE__, {:put_data, data})
  end

  def put_data_location(location, data) do
    GenServer.call(__MODULE__, {:put_data_location, location, data})
  end

  def view_data do
    GenServer.call(__MODULE__, :view_data)
  end

  def view_location_data(location) do
    GenServer.call(__MODULE__, {:view_location_data, location})
  end

  # Server (callbacks)

  @impl true
  def init(init_data) do
    {:ok, init_data, {:continue, :initialize}}
  end

  @impl true
  def handle_continue(:initialize, current_data) do
    if Application.get_env(:taiwan_buoys, :local_dev) do
      forecast_data = Forecast.get_sample_taitung_data()
      updated_data = Map.put(current_data, "taitung", forecast_data)
      {:noreply, updated_data}
    else
      Task.start(fn -> Forecast.get_all_forecast_data(&__MODULE__.put_data_location/2) end)

      updated_data =
        Enum.map(DataSources.get_locations(), fn x ->
          {x, []}
        end)
        |> Enum.into(%{})

      {:noreply, updated_data}
    end
  end

  @impl true
  def handle_call({:put_data, data}, _from, _state) do
    {:reply, data, data}
  end

  @impl true
  def handle_call({:put_data_location, location, data}, _from, current_data) do
    updated_data = Map.put(current_data, location, data)
    {:reply, updated_data, updated_data}
  end

  @impl true
  def handle_call(:view_data, _from, data) do
    {:reply, data, data}
  end

  @impl true
  def handle_call({:view_location_data, location}, _from, data) do
    {:reply, data[location], data}
  end
end
